<?php $title="Paradise Package"; ?>
<?php require_once('./header.php'); ?> 

<div class="container-fluid">
<div class="row">
  <div class="col-md-12">
    <div class="sub-gallery-nav1">
      <h1> Paradise Packages </h1>
    </div>
  </div>
</div>
</div>

 <div class="container  package-container">
	<div class="row">
        <div class="pcard" style="overflow-x: scroll;">
	<table class="plan table table-hover table-bordered table-striped text-center" width="100%">
        <thead>
            <tr>
                <th colspan="6">
                    <h2>Features</h2>
                </th>
                <th colspan="2">
                    <center>
                        <h4><b> Paradise </b></h4>
                    </center>
                </th>
                <th colspan="2">
                    <center>
                        <h4><b> Paradise Deluxe </b></h4>
                    </center>
                </th>            
                <th colspan="2">
                    <center>
                        <h4><b> Paradise Super </b></h4>
                    </center>
                </th>
            </tr>
        </thead>
        <tbody>
            <tr>
                <td colspan="6">Pick Up</td>
                <td colspan="2">✔</td>
                <td colspan="2">✔</td>
                <td colspan="2">✔</td>
            </tr>
            <tr>
                <td colspan="6">2 Weeks Storage</td>
                <td colspan="2">✔</td>
                <td colspan="2">✔</td>
                <td colspan="2">✔</td>
            </tr>
            <tr>
                <td colspan="6">Embalm & Preparation</td>
                <td colspan="2">✔</td>
                <td colspan="2">✔</td>
                <td colspan="2">✔</td>
            </tr>

            <tr>
                <td colspan="6">Casket + Casket Spray</td>
                <td colspan="2">Regular or Semi-Customized Casket</td>
                <td colspan="2">Regular or Semi-Customized Casket</td>
                <td colspan="2">Regular or Semi-Customized Casket</td>
            </tr>

            <tr>
                <td colspan="6">Arrangement</td>
                <td colspan="2">✔</td>
                <td colspan="2">✔</td>
                <td colspan="2">✔</td>
            </tr>
            <tr>
                <td colspan="6">Wreath</td>
                <td colspan="2">✔</td>
                <td colspan="2">✔</td>
                <td colspan="2">✔</td>
            </tr>

            <tr>
                <td colspan="6">Hearse</td>
                <td colspan="2">✔</td>
                <td colspan="2">✔</td>
                <td colspan="2">✔</td>
            </tr>

            <tr>
                <td colspan="6">Vault (Dovecot or Meadowrest)</td>
                <td colspan="2">Single</td>
                <td colspan="2">Single</td>
                <td colspan="2">Single</td>
            </tr>

            <tr>
                <td colspan="6">Programs</td>
                <td colspan="2">200</td>
                <td colspan="2">200</td>
                <td colspan="2">200</td>
            </tr>
            <tr>
                <td colspan="6">Prayer Cards or Book Markers</td>
                <td colspan="2">200</td>
                <td colspan="2">200</td>
                <td colspan="2">200</td>
            </tr>
            <tr>
                <td colspan="6">Buttons</td>
                <td colspan="2">100</td>
                <td colspan="2">100</td>
                <td colspan="2">100</td>
            </tr>
            <tr>
                <td colspan="6">TVJ's Death Announcement</td>
                <td colspan="2">2</td>
                <td colspan="2">2</td>
                <td colspan="2">2</td>
            </tr>
            <tr>
                <td colspan="6">Gleaner Advertisement</td>
                <td colspan="2">1</td>
                <td colspan="2">1</td>
                <td colspan="2">1</td>
            </tr>
            <tr>
                <td colspan="6">JUTC Bus</td>
                <td colspan="2">✔</td>
                <td colspan="2">✔</td>
                <td colspan="2">✔</td>
            </tr>
            <tr>
                <td colspan="6">Bands for Set up</td>
                <td colspan="2">✗</td>
                <td colspan="2">✔</td>
                <td colspan="2">✔</td>
            </tr>
             <tr>
                <td colspan="6"></td>
                <td colspan="2">
                    <a href="./request-quote.php?type=pp" class="btn btn-block btn-warning">
                        Request Quotation
                    </a>
                </td>
                <td colspan="2">
                    <a href="./request-quote.php?type=ppd" class="btn btn-block btn-warning">
                        Request Quotation
                    </a>
                </td>
                <td colspan="2">
                    <a href="./request-quote.php?type=pps" class="btn btn-block btn-warning">
                        Request Quotation
                    </a>
                </td>
            </tr>
        </tbody>
    </table>
   </div>
   </div>
</div>

<?php include './footer.php' ?>